...
class CoordinateViewController: UIViewController, CLLocationManagerDelegate, UITableViewDataSource, UITableViewDelegate
{
    ...
    //@IBOutlet weak var coordinateMapImageView: UIImageView!
    
    
    var coordinateLocationManager: CLLocationManager = CLLocationManager()
    
    let uuid = "A3E1C063-9235-4B25-AA84-D249950AADC4"
    let identifier = "coordinate-location"
    let major: CLBeaconMajorValue = 2
    //ibeacon個數
    let numberOfBeacon = 8
    ...
    let fileName = "coordinate-location_data.txt"
    ...
    override func viewDidLoad() {
        ...
        print("coordinateViewiewDidLoad")
        //顯示介面處理及初始化資料
        ...
        if CLLocationManager.isMonitoringAvailable(for: CLBeaconRegion.self){
            if CLLocationManager().authorizationStatus != CLAuthorizationStatus.authorizedAlways{
                coordinateLocationManager.requestWhenInUseAuthorization()
            }
        }
        doMonitoring(true)
    }
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        print("coordinateLViewWillDisappear")
        //doMonitoring(false)
        //coordinateLocationManager.stopUpdatingLocation()
        //coordinateLocationManager.stopRangingBeacons(satisfying: CLBeaconIdentityConstraint(uuid: UUID.init(uuidString: uuid)!, major: major))
    }
    override func viewDidAppear(_ animated: Bool) {
        super.viewDidAppear(animated)
        print("coordinateLViewDidAppear")
        clearRssiRecord(self)
        //doMonitoring(true)
        //coordinateLocationManager.startUpdatingLocation()
        //coordinateLocationManager.startRangingBeacons(satisfying: CLBeaconIdentityConstraint(uuid: UUID.init(uuidString: uuid)!, major: major))
    }
    ...
    func doMonitoring(_ state:Bool){
        ...
        coordinateLocationManager.delegate = self
        //設定region monitoring要被通知的時機
        region.notifyEntryStateOnDisplay = true
        region.notifyOnEntry = true
        region.notifyOnExit = true
        print("Coor:\(coordinateLocationManager)")
        //start monitoring
        if state {
            coordinateLocationManager.startMonitoring(for: region)
        }else{
            coordinateLocationManager.stopMonitoring(for: region)
        }
    }
    ...
    @IBAction func sectionResultButton(_ sender: Any) {
        if doPositioning{
            ...
            setSectionResultBrttonText(text: "O")
            doPositioning = true
        }
    }
